...
RSpec.describe Grouping do
  ...
  describe ".group" do
    ...
    context "when strategy is by Email1" do
      let(:strategy) { Grouping::Strategies::SameEmail.new }

      it "groups by email" do
        ...
        expect(output_io.tap(&:rewind).read).to eq(<<~CSV)
          ...
          3,Josh,Doe,(456) 789-0123,,joshd@home.com,jackd@home.com,94109
          4,Jill,Doe,(654) 987-1234,,jill@home.com,,94129
        CSV
      end
    end

    context "when strategy is by Phone1" do
      let(:strategy) { Grouping::Strategies::SamePhone.new }
    ...
    context "when strategy is by Email1 or Phone1" do
      let(:strategy) { Grouping::Strategies::SameEmailOrPhone.new }

      it "groups by email or phone" do
        described_class.group(input_io:, output_io:, strategy:, sequence:)
